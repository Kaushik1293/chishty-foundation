...
import { getSavedLanguage, setGoogleTranslateLanguage, LanguageCode } from "@/src/utils/translate";
...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const handleLangChange = (e: Event) => {
      const customEvent = e as CustomEvent<{ code: LanguageCode }>;
      if (customEvent.detail?.code) {
        const matched = languages.find((l) => l.code === customEvent.detail.code);
  ...
  const isAsgardRoute =
    pathname.startsWith("/asgard") ||
    pathname.startsWith("/login") ||
    pathname.startsWith("/events") ||
    pathname.startsWith("/asgard/partners");
  ...
  const iconBorder = isLightSurface
                      ...
                      {languages.map((lang) => (
                        <button
                          ...
                          onClick={() => {
                            setActiveLang(lang);
                            setLangOpen(false);
                            setGoogleTranslateLanguage(lang.code as LanguageCode);
                          }}
                          className={`w-full text-left px-4 py-2 text-sm font-satoshi transition-colors duration-200
                                                        ${
                                                          ...
                                                          lang.code
                                                            ? "text-light-yellow"
                                                            : "text-white/85 hover:text-light-yellow hover:bg-white/5"
                                                        }`}
                        >
                      ...
                      >
                        {isRoute ? (
                          <Link
                            href={item.href}
                            onClick={() => setMenuOpen(false)}
                            className={`block py-4 font-satoshi text-lg ${
                              active ? "text-light-yellow" : "text-white"
                            }`}
                          >
                            {item.label}
                          </Link>
