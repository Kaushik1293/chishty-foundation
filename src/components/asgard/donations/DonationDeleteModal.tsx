...
export default function DonationDeleteModal({
  ...
  return (
    <Modal
      ...
      subtitle="Are you sure you want to delete this contribution log?"
      maxWidth="max-w-md"
    >
      <div className="space-y-4 font-satoshi text-dark-green">
        <div className="p-4 rounded-xl bg-red-50 border border-red-200 flex items-start gap-3">
          ...
          <div className="text-xs text-red-700">
            ...
            <p className="mt-1">
              This action cannot be undone. This will permanently remove the donation record from your administration history.
            </p>
          </div>
        </div>
        ...
        <div className="p-3 bg-beige/60 rounded-xl border border-stroke text-xs space-y-1">
          <p>
            <strong className="text-dark-green">Donor:</strong> {donation.full_name || "Anonymous"}
          </p>
          <p>
            <strong className="text-dark-green">Amount:</strong> ₹{Number(donation.amount || 0).toLocaleString("en-IN")}
          </p>
          <p>
            <strong className="text-dark-green">Cause:</strong> {donation.category}
          </p>
          {donation.payment_id && (
            <p>
              <strong className="text-dark-green">Txn ID:</strong>{" "}
              <span className="font-mono text-[11px]">{donation.payment_id}</span>
            </p>
          )}
        </div>
